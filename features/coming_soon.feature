Feature: Move Along, Nothing To See Here
  ...
  Scenario: run the main script in a directory that is a git working dir
    ...
    When I run `git-conform` in a git working dir
    Then the exit status should be 1
     And the output should match /Coming soon! \(RuntimeError\)/

  Scenario: run the main script in a sub-directory of a git working dir

    When I run `git-conform` in a sub-directory of a git working dir
    Then the exit status should be 1
     And the output should match /Coming soon! \(RuntimeError\)/
